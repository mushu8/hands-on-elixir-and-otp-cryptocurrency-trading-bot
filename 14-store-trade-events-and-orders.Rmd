...
The `start_worker/1` is where the `Registry` will shine as we won't need to check if there is already a process running for that topic - we can leave that check to the `Registry`. If there's a process already running for that topic it will just return a tuple starting with `:error` atom:
